...
class User {
  ...
  static async authenticate(username, password) {
    ...
    if(!result.rows[0]) throw new ExpressError(`Invalid username/password`, 400);
  ...
  static async updateLoginTimestamp(username) { 
    ...
    if(!result.rows[0]) throw new ExpressError(`"${username}" not found`, 404);
  }
  ...
  static async messagesFrom(username) { 
    const result = await db.query(`
      ...
      FROM messages AS m JOIN users AS u ON m.to_username = u.username WHERE m.from_username = $1`, [username]);
  ...
  static async messagesTo(username) { 
    const result = await db.query(`
      ...
      FROM messages AS m JOIN users AS u ON m.from_username = u.username WHERE m.to_username = $1`, [username]);
    return result.rows.map(m => ({
      id: m.id,
      from_user: {
